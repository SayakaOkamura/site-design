...
/* 書かれた語から、画面の中身を組み立てる。
   ...
   本番はここを差し替える：
     ...
     return await r.json(); */
/* rl（きまり）は、書いた語をそのまま言い直さない。
   「承認」と書いたから「承認されるまで確定しない」では同語反復で、
   読んだ人が「そりゃそうだ」で終わる。ここは段2の見せ場なので、
   書いた語から一通りに決まる、しかし書いていないことを出す。
   一通りに決まらないもの（誰が・いくらから・いつまで）は GAPS 側の担当。 */
const ACTIONS = [
  // 「出す」は外した。「差異を出す」「点検日を出す」「グラフを出す」で
  // 申請者・申請日が生えていた。申請の語は 申請／届出／提出／願い で拾える。
  ...
  { f:'report', k:['集計','レポート','分析','可視化','見える化','グラフ'],
    fl:[['集計期間','今月']],
    rl:['同じ期間なら、何度出しても同じ数字になる'] },
  { f:'notify', k:['通知','メール','知らせ','連絡','アラート','リマインド'],
    fl:[],
    rl:['通知は、状態が変わったときだけ出る'] },
  ...
  { f:'bill',   k:['請求','見積','入金','支払','領収'],
    fl:[['宛先',''], ['金額','120,000', '円']],
    rl:['発行した番号は、あとから振り直さない'] },
  { f:'assign', k:['割り振','振り分','担当を','アサイン','引き継'],
    fl:[['担当','']],
    rl:['担当が決まっていないものは、未割当として残る'] }
];
...
const EGS = [
  ...
  /* **一度消した冗談の題材を、例に戻した。**（2026-09-04、008 のデモに出すとき）
     本人：**「デモの選択肢に、一回没にした面白系選択肢を入れたい」。**
     003 にあった「部長の機嫌」（IDEAS.md「削除した冗談の題材」）。
     ここは 003 のような題材ごとの作り込みを持たないので、**言葉だけ渡す。**
     それでいい——**真面目に業務システムとして受け取るのが、この題材の笑いどころ。**
     実際この一行から出るのは「機嫌登録」で、欄は 承認者／機嫌名／登録日。
     （「部長」が承認の語（ACTIONS の approve）に当たるので承認者が生える。狙って残す） */
  '部長の機嫌を記録して、話しかけていい日か通知する'
];
...
/* 例は 白紙 → 1 → 2 → … と送り、いちばん下で止まる。**戻りは ↑。**
   白紙も輪の中の一つの状態（-1）で、**↑ で必ず戻れる。**
   戻れないと、例を見たら自分では書けなくなる。

   **↓ の輪（最後 → 白紙）は外した。**（2026-09-04）本人：**「バグりやすいな」。**
   008 のデモとして全画面で出しているとき、**白紙で ↑ を押すとデモから出る**
   （本人：「一番上に着たら戻れるようにしたらいい」）。輪があると、
   ↓ で送りきった次の一押しで白紙に落ちて、**そこで ↑ を押すと例に戻るつもりで
   デモごと閉じる。**例が5本になって、送りきる回数が増えたぶん踏みやすくなった。
   止めておけば、白紙に着くのは**自分で ↑ を押して上まで戻ったときだけ**——
   そのとき ↑ が「もう上、出る」になるのは、押した本人の向きと合っている。 */
let egIdx = -1;
...
addEventListener('keydown', function(e){
  ...
  if (e.key === 'ArrowDown' || e.key === 'ArrowUp'){
    ...
    e.preventDefault();
    if (e.key === 'ArrowDown'){ if (egIdx >= last) return; pickEg(egIdx + 1); }
...
function toRingi(){
  ...
  // 件数は出さない。直後に「決まっていない欄を 3 つ」が来るので、数が混ざる。
  $('keptList').textContent = plan.title + 'の画面　一式';
  $('hold').hidden = true;
